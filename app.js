...
(() => {
  ...
  function switchTab(tabName) { document.querySelectorAll('.tab').forEach(tab => { const active = tab.dataset.tab === tabName; tab.classList.toggle('is-active', active); tab.setAttribute('aria-selected', String(active)); }); document.querySelectorAll('.panel').forEach(panel => { const active = panel.id === tabName; panel.classList.toggle('is-active',active); panel.hidden = !active; }); }

  function onQuestionSubmit(event) { event.preventDefault(); const form = event.currentTarget; const values = new FormData(form); const topic = String(values.get('topic')).trim(); const correct = values.get('outcome') === 'solved'; const timeTaken = Number(values.get('timeTaken')); if (!topic || !Number.isFinite(timeTaken) || timeTaken < 0) return; const questions = PrepStorage.getQuestions(); const index = questions.findIndex(question => nameKey(question.topic) === nameKey(topic));
  ...
  function initialize() {
    const identity = $('#identity-input'); identity.value = PrepStorage.getWhoami(); $('#offer-form select[name="day"]').innerHTML = selectOptions(Scheduler.DAYS); $('#offer-form select[name="time"]').innerHTML = selectOptions(Scheduler.TIMES);
    document.querySelectorAll('.tab').forEach(tab => tab.addEventListener('click', () => switchTab(tab.dataset.tab))); document.querySelectorAll('[data-go]').forEach(button => button.addEventListener('click', () => switchTab(button.dataset.go)));
    identity.addEventListener('input', () => { PrepStorage.setWhoami(identity.value.trim()); renderDashboard(); renderSchedule(); }); $('#question-form').addEventListener('submit',onQuestionSubmit); $('#offer-form').addEventListener('submit',onOfferSubmit);
    ...
    $('#schedule-grid').addEventListener('click', event => { const button = event.target.closest('[data-book-slot]'); if (button) tryBooking(button.dataset.bookSlot); }); $('#bookings-table-wrap').addEventListener('click', event => { const done = event.target.closest('[data-done]'); const reject = event.target.closest('[data-reject]'); const cancel = event.target.closest('[data-cancel]'); const feedback = event.target.closest('[data-feedback]'); if (done) markDone(done.dataset.done); if (reject) rejectBooking(reject.dataset.reject); if (cancel) cancelBooking(cancel.dataset.cancel); if (feedback) openFeedback(feedback.dataset.feedback); });
    $('#feedback-form').addEventListener('submit',saveFeedback); ['#feedback-cancel','#feedback-cancel-bottom'].forEach(selector => $(selector).addEventListener('click', () => $('#feedback-dialog').close())); document.querySelectorAll('.sliders input').forEach(input => input.addEventListener('input', () => input.closest('label').querySelector('output').value = input.value)); renderAll();
  }
  initialize();
})();
